...
| AC-5 | Output loads into a PCSE `WeatherDataProvider` unchanged | container-native columns and units; `check_weather.py` `build_provider` | `Wofost72_PP` maize run on the Iowa series to maturity: sown 2026-05-01, anthesis 06-30, maturity 08-13, TWSO 10,926 kg/ha, LAImax 4.22 -- potential production, so no water, nutrient or pest limitation (labelled under brief 0002); six variables read back unchanged; PCSE's own unit strings match | pass |
